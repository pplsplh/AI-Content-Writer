...
export default function Home() {
  ...
  return (
    <main className="min-h-screen bg-[#080b14] text-white flex flex-col items-center justify-start px-4 py-16">
      ...
      <div className={`relative w-full ${result ? 'max-w-7xl' : 'max-w-2xl'} transition-all duration-500`}>
        {/* Header */}
        <div className="text-center mb-10">
          ...
          <p className="text-slate-400 text-base max-w-md mx-auto leading-relaxed">
            Sentuhan AI dengan rasa manusiawi. Ciptakan narasi yang hangat, bermakna, dan membekas di hati.
          </p>
        </div>

        <div className={result ? 'grid grid-cols-1 lg:grid-cols-[2fr_3fr] gap-6 items-start' : ''}>
        {/* Form Card */}
        <div className="bg-white/[0.03] border border-white/10 backdrop-blur-xl rounded-3xl p-8 shadow-2xl shadow-black/40 space-y-6">
          ...
          <div>
            <div className="flex items-center justify-between mb-3">
              ...
              <span className="text-xs text-slate-600">opsional</span>
            </div>
            <div className="grid grid-cols-4 gap-2">
              {MOODS.map((m) => (
                <button
                  ...
                  className={`flex flex-col items-center gap-1 py-2.5 px-1 rounded-xl text-xs font-semibold transition-all duration-200 border ${
                    mood?.value === m.value
          ...
          <button
            onClick={generate}
            disabled={loading}
            className="relative w-full py-4 rounded-xl font-bold text-sm tracking-wide overflow-hidden transition-all duration-300 disabled:opacity-60 disabled:cursor-not-allowed group"
          >
        ...
        {/* Result - right column */}
        {result && (
          <div className="bg-white/[0.03] border border-white/10 rounded-xl overflow-hidden sticky top-8">
